...
@interface RenderBlurView () <UICollectionViewDataSource, UICollectionViewDelegate>
    ...
    if (_datas == nil) {
        ...
        for (NSArray *array in tmpArr) {
            ...
            for (NSDictionary *dic in array) {
                FoundCellModel *foundModel = [FoundCellModel foundCellModelWihtDict:dic];
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.cancelBtn.frame = CGRectMake((AppWidth - 30) / 2, AppHeight - 64 - 50, 30, 30);
        ...
        CGFloat marginLR = (AppWidth - 3 * itemWH) / 4;
        ...
        [self.collectionView registerNib:[UINib nibWithNibName:@"BlurCell" bundle:nil] forCellWithReuseIdentifier:@"blurCell"];
    }
...
{
    RenderBlurView *view = [[self alloc] init];
    [view setImage:image];
    return view;
}
...
{
    //取出cell对应的模型
    FoundCellModel *foundModel = self.datas[0][indexPath.row];
    
    //取出cell
    BlurCell *cell = [BlurCell blurCellWithCollectionView:collectionView forIndexPath:indexPath];
...
{
    BlurCell *cell = (BlurCell *)[collectionView cellForItemAtIndexPath:indexPath];
...
- (void)layoutSubviews
{
    ...
    self.collectionView.frame = CGRectMake(0, self.bounds.origin.y + 60, AppWidth, self.bounds.size.height - 60 - 50);
}
