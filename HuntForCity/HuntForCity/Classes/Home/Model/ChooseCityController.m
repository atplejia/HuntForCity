...
@interface ChooseCityController ()<CityGroupCellDelegate,UISearchBarDelegate,CLLocationManagerDelegate>
/**
 *  记录所有城市信息，用于搜索
 */
...
- (NSMutableArray *) localCityData
{
    if (_localCityData == nil) {
        ...
        if (self.locationCityID != nil) {
            City *city = nil;
            for (City *item in self.recordCityData) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    CityGroup *group = [self.cityDatas objectAtIndex:section - 3];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cityCell];
    CityGroup *group = [self.cityDatas objectAtIndex:indexPath.section - 3];
...
- (CGFloat) tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        return [CityGroupCell getCellHeightOfCityArray:self.commonCityData];
    }
...
#pragma mark CityGroupCellDelegate
