...
@interface DetailViewController ()<UITableViewDataSource, UITableViewDelegate, SelectViewDelegate, DetailFootViewDelegate,MAMapViewDelegate,UIScrollViewDelegate,UIActionSheetDelegate>
...
/** 顶部的图片scrollView */
@property (nonatomic, strong) ScrollHeadView          *topScrollView;
/** 返回按钮 */
...
@property (nonatomic, strong) DetailRnmdTableHeadView *tableHeadView;
/** 所有rmdTableView的foodView */
@property (nonatomic, strong) DetailFootView          *footView;
/** infoTableView的footView */
@property (nonatomic, strong) DetailFootView          *footView1;
/** 信息tableView */
@property (nonatomic, strong) UITableView                *infoTableView;
/** 记录当前展示的tableView 计算顶部topView滑动的距离 */
@property (nonatomic, weak  ) UITableView                *showingTableView;
/** 去过和收藏view */
@property (nonatomic, strong) BeenAndCollectView      *beenAndCollectView;
/** 推荐tableView的数据源 */
...
- (NSArray *)infoDatas
{
    if (_infoDatas == nil) {
        ...
        for (NSDictionary *dict in arr) {
            ...
            InfoModel *model = [InfoModel infoModelWithDict:dict];
...
- (DetailModel *)details
{
    if (_details == nil) {
        ...
        _details = [DetailModel detailModelWith:dict[@"body"]];
    }
    return _details;
}
...
- (NSArray *)rmdDatas
{
    ...
    if (_rmdDatas == nil) {
        ...
        for (NSDictionary *dict in tmp) {
            RmdCellModel *cellModel = [RmdCellModel rmdCellModelWithDict:dict];
...
-(void)setUI{
    //隐藏系统的导航条，由于自定义的动画，自定义一个view来代替导航条
    [self.navigationController setNavigationBarHidden:YES animated:YES];
    
    //将view添加到scrollview的内容上
    self.automaticallyAdjustsScrollViewInsets = NO;
    //设置背景颜色
    self.view.backgroundColor = [UIColor whiteColor];
    //初始话最底部的scrollview，放在tableview用
    ...
    self.backgroundScrollView.contentSize = CGSizeMake(AppWidth * 2, 0);
    ...
    //添加底部footView
    self.footView = [DetailFootView detailFootView];
    self.footView.delegate = self;
    self.rmdTableView.tableFooterView = self.footView;
    
    //将控制器添加给self
    self.infoTableView = [[UITableView alloc] initWithFrame:CGRectMake(AppWidth, 0, AppWidth, AppHeight) style:UITableViewStylePlain];
    ...
    //初始化footView1
    self.footView1 = [DetailFootView detailFootView];
    ...
    self.topScrollView = [ScrollHeadView scrollHeadViewWithImages:arr];
    [self.topScrollView setFrame:CGRectMake(0, 0, AppWidth, ScrollHeadViewHeight)];
    ...
    _selectView = [SelectView selectViewWithisShowComment:NO];
    _selectView.delegate = self;
    _selectView.frame = CGRectMake(0, CGRectGetMaxY(self.topView.frame),
                                   AppWidth, SelectViewHeight);
    [self.view addSubview:_selectView];
    
    //添加推荐tableView的tableHeadView
    self.tableHeadView = [DetailRnmdTableHeadView detailRnmdTableHeadView];
    ...
    self.beenAndCollectView = [BeenAndCollectView beenAndCollectView];
    CGFloat beenAndCollectViewH = 45;
    self.beenAndCollectView.frame = CGRectMake(0, AppHeight - beenAndCollectViewH, AppWidth, beenAndCollectViewH);
    [self.view addSubview:self.beenAndCollectView];
    
    //初始化地图
//    [MAMapServices sharedServices].apiKey = @"2e6b9f0a88b4a79366a13ce1ee9688b8";
    self.mapView = [[MAMapView alloc] initWithFrame:CGRectMake(0, 0, AppWidth, 220)];
    self.mapView.delegate = self;
    self.mapView.showsScale = NO;
    self.mapView.showsCompass = NO;
    self.mapView.showsUserLocation = YES;
    self.mapView.logoCenter = CGPointMake(AppWidth - self.mapView.logoSize.width + 5, 220 - self.mapView.logoSize.height);
...
{
    //初始化山寨导航条
    self.naviView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, AppWidth, 64)];
    self.naviView.backgroundColor = GolbalGreen;
    ...
    self.sharedBtn.frame = CGRectMake(AppWidth - 36, 34, 26, 17);
    ...
    [self.titleLabel setFrame:CGRectMake(30, 32, AppWidth - 35 - 50, 25)];
    self.titleLabel.textAlignment = NSTextAlignmentLeft;
    self.titleLabel.font = [UIFont systemFontOfSize:19];
    self.titleLabel.text = @"深圳南山区世界之窗";
    ...
    //添加副标题
    self.subTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(52, 60, AppWidth - 180, 20)];
    self.subTitleLabel.textAlignment = NSTextAlignmentLeft;
    self.subTitleLabel.textColor = [UIColor whiteColor];
    self.subTitleLabel.font = [UIFont systemFontOfSize:16];
    self.subTitleLabel.text = @"深圳";
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    if (scrollView == self.rmdTableView || scrollView == self.infoTableView) {//说明是tableView在滚动
        ...
        if (offsetY >= -(naviH + SelectViewHeight)) {
            self.selectView.frame = CGRectMake(0, naviH, AppWidth, SelectViewHeight);
        } else {
            self.selectView.frame = CGRectMake(0, CGRectGetMaxY(self.topView.frame), AppWidth, SelectViewHeight);
        }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == self.rmdTableView) {
        //推荐tableView
        RmdCellModel *rmdCellModel = self.rmdDatas[indexPath.row];
        ...
        //信息tableVIew
        InfoModel *model = self.infoDatas[indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView == self.rmdTableView) {
        ...
        NSString *ch = rmdCellModel.ch;
        if (ch) {
            return [RmdTextCell cellWithTabelView:tableView rmdCellModel:rmdCellModel];
        } else {
            return [RmdPicCell cellWithTabelView:tableView rmdPicModel:rmdCellModel];
        }
    }
    
    InfoCell *cell = [InfoCell infoCellWithTableView:tableView];
...
#pragma DetailFootViewDelegate 底部收藏用户的点击事件
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (buttonIndex != actionSheet.numberOfButtons - 1) {
        ...
        InfoModel *model = self.infoDatas[1];
